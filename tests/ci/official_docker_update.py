...
def update_docs(repos: LibraryRepos, dry_run: bool = True) -> None:
    ...
    run(
        f"{GIT_PREFIX} push --set-upstream-to=origin/{LIBRARY_BRANCH}",
        dry_run,
        cwd=docs_dir,
    )
...
def update_library_images(repos: LibraryRepos, dry_run: bool = True) -> None:
    ...
    generate_tree_cmd = (
        f"{executable} tests/ci/official_docker.py generate-tree --build "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        "Script to update all docker library repositories",
